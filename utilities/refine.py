...
def refine_mesh(source_mesh_V, source_mesh_F, target_mesh_V, use_geod=True):
    global device
    dg = DeformationGraph()
    source_mesh_V, source_mesh_F, target_mesh_V = torch.from_numpy(source_mesh_V), torch.from_numpy(source_mesh_F), torch.from_numpy(target_mesh_V)

    if use_geod:
        geod = compute_geodesic_distmat(source_mesh_V.cpu().numpy(), source_mesh_F.cpu().numpy())      
        dg.construct_graph(source_mesh_V.cpu(), source_mesh_F, geod, device)
    else:
        geod = torch.cdist(source_mesh_V, source_mesh_V, p=2.0).cpu().numpy()
        dg.construct_graph_euclidean(source_mesh_V.detach().cpu(), geod, device)
    num_nodes = dg.nodes_idx.shape[0]

    opt_d_rotations = torch.zeros((1, num_nodes, 3)).double().to(device) # axis angle 
    opt_d_translations = torch.zeros((1, num_nodes, 3)).double().to(device)
    ...
    while True:
        ...
        target_mesh_V_nodes = target_mesh_V_T
        new_source_mesh_V_nodes = new_source_mesh_V
